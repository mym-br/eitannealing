          ...
          for(unsigned long i = 0; i < j; i++) {
            ...
            for(typename Eigen::SparseMatrix<scalar>::InnerIterator it(a,i); it; ++it) {
              ...
              if(ll != buildingL.end()) { // There are remaining elements, get the nl largest
                ...
                // Now selectedL has the nl largest elements. Sort them according to index
                std::sort(selectedL.begin(), selectedL.end(), [](const i_c &a, i_c const &b){return a.first < b.first;});
                // Fill new L column
              }
            }
          }
